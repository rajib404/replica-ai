...
export function useVoiceSocket({
  ...
  const connect = useCallback(() => {
    ...
    const ws = new WebSocket(`${WS_BASE}/ws/voice/${ownerId}?token=${token}`);
    ...
    ws.onmessage = (event) => {
      ...
      try {
        ...
        switch (data.type) {
          ...
          case 'audio_out':
            // Store metadata, actual audio comes as next binary frame
  ...
  }, [ownerId, enabled, onTranscript, onResponseText, onAudioOut, onDone, onStatusChange, onError]);
